...
public class Cache<K, V> implements Map<K, V> {
    ...
    public synchronized V get(Object key) {
        ...
        // and re-add it at the front of the list.
    ...
    public synchronized void setMaxCacheSize(int maxCacheSize) {
        ...
        // size. If so, we need to remove infrequently used items.
        cullCache();
    }
    ...
    protected synchronized void cullCache() {
        ...
        if (map.size() > maxCacheSize) {
            // First, remove any old entries to see how much memory that frees.
            deleteExpiredEntries();
            // Next, remove the least recently used elements until 10% of the cache
            // has been freed.
     ...
     * are public, which allows very fast remove operations when one has a
     ...
     * be performed to find the correct node to remove.
     *
